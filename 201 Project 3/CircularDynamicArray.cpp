...
using namespace std;
#ifndef _CIRCULAR_DYNAMIC_ARRAY
#define _CIRCULAR_DYNAMIC_ARRAY
...
private:
  ...
  int _index(int index) {
    return (_start + index + _capacity)%_capacity;
  }
...
public:
  ...
  CircularDynamicArray(const CircularDynamicArray<elmtype> &original){
    ...
    for (int i = 0; i < _end; i++){
      //(*this)[i] = original[i];
      _array[i] = original._array[_index(i)];
    }
  }
...
//      (*this)[i] = original._array[(original._start+i+_capacity)%_capacity];
      _array[i] = original._array[(original._start+i+_capacity)%_capacity];
      //_array[i] = original[i];
    }
    return *this;
  }
  ...
  ~CircularDynamicArray()
  {
    //cout << "CDA destructor called" << endl;
    if (_array != nullptr){
      delete[] _array;
    }
    _array = nullptr;
  ...
  void delEnd(){
    ...
    if (length() > 0 && _capacity/length() >= 4){
      _shrink();
      /*elmtype* temp = _resize(_array, capacity()/2);
      delete[] _array;
      _array = temp;
